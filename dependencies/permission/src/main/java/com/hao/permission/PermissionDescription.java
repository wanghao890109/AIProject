...
public final class PermissionDescription implements OnPermissionDescription {
    ...
    private Map<String, String> customDescList;

    /**
     * @param desc 自定义权限描述文案
     */
    public PermissionDescription(Map<String, String> desc) {
        customDescList = desc;
    }
    ...
    public void askWhetherRequestPermission(@NonNull Activity activity,
        ...
        XMPermissionsHelper.setPermissionCancel(activity, null);
        showDialog(activity, activity.getString(R.string.common_permission_description_title),
                ...
                activity.getString(R.string.common_permission_cancel), (dialog, which) -> {
                    dialog.dismiss();
                    XMPermissionsHelper.setPermissionCancel(activity, requestList);
    ...
    private String generatePermissionDescription(@NonNull Activity activity, @NonNull List<IPermission> requestList) {
        if (customDescList != null && !customDescList.isEmpty()) {
            //获取自定义文案
            IPermission firstPermission = requestList.get(0);
            String firstKey = firstPermission.getPermissionName();
            String customDesc = customDescList.get(firstKey);
            if (!TextUtils.isEmpty(customDesc)) {
                Integer permissionNameStringId = PermissionConverter.getPermissionNickNameStringId(activity, firstPermission);
                if (permissionNameStringId != null && permissionNameStringId > 0) {
                    String permissionNickName = activity.getString(permissionNameStringId);
                    return permissionNickName + activity.getString(R.string.common_permission_colon) + customDesc;
                }
            }
        }
