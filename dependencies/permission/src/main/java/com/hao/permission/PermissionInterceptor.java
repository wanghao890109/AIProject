...
public final class PermissionInterceptor implements OnPermissionInterceptor {
    ...
    public void onRequestPermissionEnd(@NonNull Activity activity, boolean skipRequest,
        ...
        List<IPermission> cancelPermissions = XMPermissionsHelper.getPermissionCancel(activity);
        if (cancelPermissions != null && requestList != null) {
            if (XMPermissionsHelper.equalsPermission(cancelPermissions, requestList)) {
                //该权限已经被取消
                return;
            }
        }
    ...
    private void showPermissionSettingDialog(@NonNull Activity activity,
        ...
        String cancelButtonText = activity.getString(R.string.common_permission_cancel);
