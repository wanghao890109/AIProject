...
public class DrawableTextView extends AppCompatTextView {
    ...
    public DrawableTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        drawableTint =  array.getColorStateList(R.styleable.DrawableTextView_drawableTint);
    ...
    private void applyTint(Drawable[] drawables) {
        if (drawables == null) {
            return;
        }
        for (int i = 0; i < drawables.length; i++) {
            applyTint(drawables[i]);
        }
    }
    ...
    private void applyTint(Drawable drawable) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            drawable.setTintList(drawableTint);
        }
    }
