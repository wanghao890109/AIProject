package com.hao.ui.widget.grid.adapter;
...
public class BaseGridAdapter<T> extends BaseAdapter {
    ...
    protected GridPageEntity mEmoticonPageEntity;
    ...
    protected GridClickListener mOnEmoticonClickListener;

    public BaseGridAdapter(Context context, GridPageEntity emoticonPageEntity, GridClickListener onEmoticonClickListener) {
        ...
        this.mDefalutItemHeight = this.mItemHeight = (int) 50;
        this.mData.addAll(emoticonPageEntity.getEmoticonList());
        checkDelBtn(emoticonPageEntity);
    }

    private void checkDelBtn(GridPageEntity entity) {
        GridPageEntity.DelBtnStatus delBtnStatus = entity.getDelBtnStatus();
        if (GridPageEntity.DelBtnStatus.GONE.equals(delBtnStatus)) {
            return;
        }
        if (GridPageEntity.DelBtnStatus.FOLLOW.equals(delBtnStatus)) {
            mDelbtnPosition = getCount();
            mData.add(null);
        } else if (GridPageEntity.DelBtnStatus.LAST.equals(delBtnStatus)) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = mInflater.inflate(R.layout.grid_base_item, null);
